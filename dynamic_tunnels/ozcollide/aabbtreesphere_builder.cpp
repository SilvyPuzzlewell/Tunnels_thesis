...
AABBTreeSphere_Builder::~AABBTreeSphere_Builder()
{
    // tree has to be deleted elsewhere
    tree_ = NULL;
//std::cerr << "Builder DESC!!!!!\n\n";
}
...
//    std::cerr << "First iter size " << items_.size() << ", nbSpheres=" << nbSpheres << "\n";
...
//    item->spheres.clear(); // VV
//	delete item;


    //VV
//    std::cerr << "Size is items_ is " << items_.size() << "\n";
//    std::cerr << "Capacity of items_ is " << items_.capacity() << "\n";
    for(int i=0;i<items_.size();i++) {
//        std::cerr << "C[" << i << "]";
        items_[i]->spheres.clear();
        delete items_[i];
    }
	items_.clear();
...
void AABBTreeSphere_Builder::workOnItem(WorkingItem &item, int leafDepth)
{
    ...
    //        left->spheres.reserve( left->spheres.size() ); // VV
		}
		else if (!left && right) {
			left = new WorkingItem();
      ...
      //      right->spheres.reserve( right->spheres.size() ); // VV
		}
	}
