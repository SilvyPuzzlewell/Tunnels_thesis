...
bool AABBTreeSphere::isCollideWithSphere(const Sphere &_sphere)
{
	nbColls_ = 0;
	sphere_ = _sphere;
    Vec3f a= _sphere.center;
    Vec3f b= _sphere.center;
    a.x-=_sphere.radius;
    a.y-=_sphere.radius;
    a.z-=_sphere.radius;
    ...
    box_.setFromPoints(a,b); 
	return collideWithSphere(*root_);
}

bool AABBTreeSphere::collideWithSphere(const AABBTreeNode &_node)
{
	if (_node.aabb.isOverlap(box_) == true) {
		if (!_node.left && !_node.right) {
			// reach a leaf
			...
			for (int i = 0; i < nbSpheres; i++) {
				...
				const Sphere &s = pl.spheres[i];
//				return testIntersectionSphereBox(s, box_);
                return s.isCollide(sphere_);
			}
		}
		else {
			if (_node.left) {
				if (collideWithSphere(*_node.left) == true)
					return true;
			}
			if (_node.right) {
				if (collideWithSphere(*_node.right) == true)
					return true;
			}
		}
	}
