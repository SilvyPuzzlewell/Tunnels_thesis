...
void build_tree(bool is_global);
...
std::vector <Path*> paths;        //here are the fpund paths kept, all nodes in them are copied, therefore there isn't problem with deleting
...
std::map <int, shared_ptr<vertex>> local_priority_kdTree_coordinates; //exists only because dumbass MPNN doesn't support delete, kepts coordinates of local kd Tree
std::map <int, vertex*> global_tree_points;                //to:do
...
bool contains_key(int index, std::map<int, vertex*>& map){
  std::map<int, vertex*>::iterator it;
  it = map.find(index);
  return it != map.end();
}

vertex* load_next_vertex(map <int, vertex*>& graph_points, int index){
  vertex* cur;
  if(contains_key(index, local_priority_kdTree_coordinates)){    
    cur = local_priority_kdTree_coordinates[index];
  } else {
    cur = graph_points[index];
  }
  return cur;
}
...
Path* backtrack(map <int, vertex*>& graph_points, int path_end_index){ 
  vertex* cur = load_next_vertex(graph_points,path_end_index);
  ...
  while(cur->parent_index != -1){
    ...
    cur = load_next_vertex(graph_points, cur->parent_index);
...
std::map<int, vertex*>& init_start(std::map <int, vertex*>& graph_points){
  ...
  local_priority_kdTree_coordinates.insert(std::pair<int,vertex*>(0, start));
  
  tree_index = 0;
  add_to_tree(start->location_coordinates, tree_index, local_priority_kdTree);
  tree_index++; 
  found = false;
  return graph_points;
}
...
std::map<int, vertex*>& find_path(std::map<int, vertex*>& graph_points, ofstream* stats_filename){
  //chrono stuff is used to measure time, in which this main part of algorithm ran
  high_resolution_clock::time_point t1 = high_resolution_clock::now();
  ...
  for(int i = 0; i < iterations; i++){
    ...
    shared_ptr<vertex> new_vertex = make_shared<vertex>(sampled_coords, parent_index, tree_index, probe_radius, get_current_frame());
    ...
    /*
    ------
    this part tries to add vertex into priritized tree belonging to the frames' tree structure.
    If fails, attempt to connect it to global structure is made, and the new found node is then placed
    into frames' local structure. In first frame, global structure doesn't exist, therefore it's ignored 
    */ 
    vertex* parent = local_priority_kdTree_coordinates[parent_index];
    
    graph_points = step(graph_points, new_vertex, parent, min_step, 1, parent_index); //function for adding next point to graph points and sending signals to end iterating, use whichever you want
                                                                                      //new_vertex is expected to be deleted if step_success_flag is set false, new vertex is added to                
                                                                                      //both local and global coordinate representation, and only to local kd tree, the local kd tree is kept because of
                                                                                      //the need to track local kd tree when deleting parts of it
    
    if(step_success_flag == false && get_current_frame() != 1){  //first frame obviously doesn't have global tree, and the attempt to connect to local tree was unsuccessful
      int parent_index = find_nearest_vertex(sampled_coords, global_kdTree); //now the parent is searched in global tree, which contains nodes from previous iterations
      vertex* new_vertex = new vertex(sampled_coords, parent_index, tree_index, probe_radius, get_current_frame());  
...
void rebuild_kd_tree(std::map<int, vertex*>& graph_points, bool is_initialized, bool is_global, bool copy_stuff){
  if(is_initialized){
    if(is_global) {delete global_kdTree;}
    else {delete local_priority_kdTree;}
  }
  build_tree(is_global);
  
  if(copy_stuff && is_global){
  ...
  else if(copy_stuff && !is_global){
    for (std::map<int, vertex*>::iterator iterator = local_priority_kdTree_coordinates.begin(); iterator != local_priority_kdTree_coordinates.end(); iterator++){
      add_to_tree(iterator->second->location_coordinates, iterator->second->index, local_priority_kdTree);
    }
  }
}
...
void cut_subtree_in_main_tree(int index, std::map<int ,vertex*>& tree, bool has_debugging_output, bool is_global){
  int prev_index = tree[index]->parent_index;
  vertex* parent = tree[prev_index];
  bool found = false;
  ...
  cut_subtree_in_main_tree_recursion(index, tree, has_debugging_output);
  rebuild_kd_tree(tree, true, is_global, true);
}
...
int copies_count = 0;
void center_tunnel(Path* path, std::map<int ,vertex*>& graph_points, bool is_global){
  std::map<int, vertex*>::iterator iterator;

  int counter = 0;
  int index = -1;
  int last_cur_frame_index = -1;
  ...
  for(iterator = path_vertices.begin(); iterator != path_vertices.end(); iterator++){
    ...
    */
    ...
    if(!is_global){
      exists_in_path = local_priority_kdTree_coordinates[cur_node->index]->is_in_path();
    }
    if(exists_in_path != -1){
      vertex* copied_node = paths[exists_in_path]->get_node_pointer(cur_node->index)->copy();
      path_vertices[cur_node->index] = copied_node;
      copied_node->parent_index = cur_node->parent_index; copied_node->children_indices = cur_node->children_indices;
      copies_count++;
    ...
    if(cur_node->get_last_valid_frame() == get_current_frame()){
      last_cur_frame_index = cur_node->index;
    }
    delete [] direction_vector;
    ...
    */
    ...
    if(path_vertices[index]->radius > test_sphere_radius){
      int child_index_global = path->get_child_index(index);
      int child_index_local = path->get_child_index(last_cur_frame_index);

      if(get_current_frame() > 1){
        cut_subtree_in_main_tree(child_index_global, graph_points, false, GLOBAL);
      }
      cut_subtree_in_main_tree(child_index_local, local_priority_kdTree_coordinates, false, LOCAL); 
      cut_path(child_index_global, path_vertices); 
      break;}; //to_do
    if(cur_node->children_indices.size() > 0){
      index = cur_node->children_indices.back();
    }
...
void cut_tunnel(Path* path, std::map<int, vertex*>& graph_points){
  ...
  std::vector<int> vertex_indices_to_be_deleted_local;
  ...
  if(vertex_indices_to_be_deleted.size() > 1){ //cut if something was deleted from path
    cut_subtree_in_main_tree(vertex_indices_to_be_deleted[vertex_indices_to_be_deleted.size() - 2], local_priority_kdTree_coordinates, false, LOCAL); //from element before the last
  }
}
...
int path_optimization(Path* path, std::map<int, vertex*>& graph_points, bool is_global){
  //path = smoothing(path);
  high_resolution_clock::time_point t1 = high_resolution_clock::now();
 
  smoothing(path);
  center_tunnel(path, graph_points, is_global);
...
void label_used_nodes(Path* path, std::map<int, vertex*>& main_tree, int path_index){
  for(std::map<int, vertex*>::iterator iterator = path->get_vertices().begin(); iterator != path->get_vertices().end(); iterator++){
    main_tree[iterator->second->index]->set_in_path(path_index);
  }
}

std::map<int, vertex*>& step(std::map<int, vertex*>& graph_points, shared_ptr<vertex> new_vertex, vertex* parent, double goal_distance, 
  double collision_check_step, int parent_index){
 // print_vertex_coordinates(destination);
  ...
  local_priority_kdTree_coordinates.insert(std::pair<int,vertex*>(tree_index, new_vertex));
  ...
  tree_index++;
    ...
    path_optimization(path, graph_points, false);
    ...
    */
    ...
    if(is_duplicate == -1){
      ...
      label_used_nodes(path, local_priority_kdTree_coordinates, paths.size() - 1);
...
MPNN::MultiANN<double>* new_frame_initalisation(map<int, vertex*>& graph_points){
  ...
  if(get_current_frame() == 2){
    rebuild_kd_tree(graph_points,false, GLOBAL, true);
  } else {
    rebuild_kd_tree(graph_points,true, GLOBAL, true);
  }

  rebuild_kd_tree(graph_points, true, LOCAL, false);                //deletes last frame's priority tree
  delete_vertex_pointer_vector(local_priority_kdTree_coordinates); //deletes storage for nodes in previous local tree
...
{  
  ...
  int tree_size_at_finish = local_priority_kdTree_coordinates.size();
